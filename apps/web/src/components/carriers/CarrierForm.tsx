...
export const CarrierForm: React.FC<CarrierFormProps> = ({
  ...
  const { data: comunaData } = useComuna(carrier?.comunaId || "", {
    enabled: isEditing && !!carrier?.comunaId,
  });
  ...
  const {
    ...
    setValue,
    reset,
  } = useForm<CreateCarrierDto & { comuna?: Comuna | null }>({
    defaultValues: {
      esPersona: false,
    },
  });
  ...
  useEffect(() => {
    if (isEditing && carrier) {
      reset({
        ...
        esPersona: carrier.esPersona,
        comuna: comunaData || null,
      });
    }
  }, [isEditing, carrier, comunaData, reset]);

  const handleFormSubmit = (data: any) => {
    const submitData: CreateCarrierDto = {
      nombre: data.nombre || "",
  ...
  const title = isEditing ? "Editar Carrier" : "Crear Nuevo Carrier";
  const description = isEditing
    ? "Modifica la información del carrier"
    : "Completa la información del nuevo carrier";
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex items-center space-x-2 md:col-span-2 p-4 bg-[#EFF4F9] rounded-lg">
                ...
                <p className="text-xs text-[#798283]/70 ml-2">
                  {isPersona
                    ? "Carrier es persona natural (no requiere razón social)"
                    : "Carrier es empresa (puede incluir razón social)"}
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="carrier@ejemplo.com"
                  disabled={isLoading}
                  className="border-[#798283]/30 focus:ring-[#D42B22] focus:border-[#D42B22]"
                />
                {errors.email && (
